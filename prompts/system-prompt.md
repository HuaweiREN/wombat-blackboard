...
- 不要使用任何HTML标签（`<br/>`、`<div>`、`<span>`、`<b>`等均不允许）。需要换行时用逗号或分号分隔。
...
- **严禁在 Mermaid 代码块内出现裸文本行**：每一行都必须是合法的 Mermaid 语句。解释性文字放在 `Note` 中，且 Note 内容写在冒号后的同一行（用逗号、句号或中文标点分隔，不使用 HTML 标签），不要另起多行。时序图的连线标签也写在同一行。
